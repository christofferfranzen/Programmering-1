...
class Temperatures2
{
	...
	public static double min (double[] temp)
	{
		...
		for (int reading = 2; reading <= temp.length-1; reading++){
			if (temp[reading] < min){
				min = temp[reading];
			}
		}
	...
	public static double sum (double[] temp)
	{
		...
		for (int reading = 2; reading <= temp.length-1; reading++){		// 2 för att sumTemp = sumT[1] redan, så att 1 behöver inte adderas
			sum += temp[reading];
		}
